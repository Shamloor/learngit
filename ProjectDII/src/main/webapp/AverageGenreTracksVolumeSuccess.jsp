...
<script type="text/javascript">
    ...
    //将后台的数据取出
    var average2 = JSON.parse(JSON.stringify(${average2}));
   
    var list = average2.rows;
    var dataset = [];
    var genre = [];
    var tcount = [];
    ...
    for (var item of list) {
        genre.push(item.rowkey);
        tcount.push(item.value);
    ...
    myChart.on('click', function (params) {
        console.log(genre[Math.max(params.dataIndex - zoomSize / 2, 0)]);
        myChart.dispatchAction({
            type: 'dataZoom',
            startValue: genre[Math.max(params.dataIndex - zoomSize / 2, 0)],
            endValue:
                genre[Math.min(params.dataIndex + zoomSize / 2, data.length - 1)]
        });
    });
